                ...
                if (result.isConfirmed) {
                    ...
                    try {
                        ...
                        if (error.code === 'auth/invalid-verification-code') {
                            errorMessage = 'Invalid verification code. Please check the code you entered and try again.';
                        } else if (error.code === 'auth/missing-verification-code') {
                            errorMessage = 'Missing verification code. Please enter the code sent to your phone and try again.';
                        } else if (error.code === 'auth/quota-exceeded') {
                            errorMessage = 'Quota exceeded. Please try again later.';
                        } else if (error.code === 'auth/user-disabled') {
                            errorMessage = 'User account has been disabled. Please contact support to resolve this issue.';
                        } else if (error.code === 'auth/user-not-found') {
                            errorMessage = 'User account not found. Please check the phone number you entered and try again.';
                        }
                        // Add more specific cases as needed
    ...
    return (
        <main className="flex h-screen justify-center items-center antialiased text-gray-800">
            <div className="bg-white p-8 shadow-md rounded-md flex flex-col items-center sm:w-full md:max-w-md">
                ...
                <button onClick={handleGoogleSignIn} className="hidden items-center bg-white dark:bg-gray-900 border border-gray-300 rounded-lg shadow-md px-6 py-2 text-sm font-medium text-gray-800 dark:text-white hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                ...
                <div className='hidden border-t-2 border-zinc-300 mt-8 mb-3 w-full text-center'>
                    <span className='-mt-[14px] block w-max bg-white mx-auto px-2'>or</span>
                </div>

                <div className='flex flex-col items-center w-full'>
                    <label className="block mb-2 mr-auto" htmlFor="phone">
                        Phone Number:
                    </label>
                    <span className='text-red-700 text-xs mb-1'>
                        {errorPhone}
